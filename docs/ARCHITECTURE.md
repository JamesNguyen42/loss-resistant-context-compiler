...
manifest schema `0.7` requires an immutable adapter revision, a retained
...
network-isolation evidence for claim eligibility. It also binds a pre-existing
service's PID creation identity and executable digest and records sampled
Windows working set or Linux RSS plus its ceiling. External scoring
additionally compares all of those fields, the lock/network-evidence digests,
service metric/executable digest/ceiling, and retained runner limits, including
the enforcement polling cadence, with the frozen protocol.
