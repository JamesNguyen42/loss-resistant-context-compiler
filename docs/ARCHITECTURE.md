...
manifest schema `0.8` requires an immutable adapter revision, a retained
...
retained adapter entrypoint, requires the command to reference that file, and
binds the canonical command digest. It also binds a pre-existing service's PID
creation identity and executable digest and records sampled Windows working set
or Linux RSS plus its ceiling. External scoring additionally compares all of
those fields, the lock/entrypoint/command/network-evidence digests, service
