...
| SWE-bench prediction omission, duplicate-id collapse, denominator shrinkage, substitution, or patch leakage | The source-only prediction boundary replays the exact source/key task projection and every claimed successful preparation, reconciles an independently supplied expected code/model/agent/prompt/tool/controller identity, and handles only opaque candidate ids over the complete selected physical order. Missing/duplicate/unexpected/invalid/oversized/unprepared output becomes a closed nonprediction or bounded protocol row; contiguous capture ordinals plus separate count, aggregate-capture, per-patch, ledger, and JSONL limits bound construction and replay work. Exactly one canonical official JSONL line is written per selected row through exclusive guarded output outside prepared trees and mirrors; live readers bind the opened descriptor to the initial/installed identity, reject A-B-A substitution, and remove only the identity-matched owned file after a late failure. Exact patch, line, and whole-file byte counts and SHA-256s support live replay; rejected raw content, gold, keys, and runtime paths are not serialized; execution, grading, score, usefulness, and claim-ready fields are fixed false | The coordinator and Python process can fabricate a new internally consistent ledger and JSONL; supplied system digests are not signatures, producer attestation, or proof of loaded code. Because rejected raw captures are discarded, invalid/duplicate/unexpected protocol evidence remains producer-asserted unless its bytes came through the separate controller-run boundary. Valid patches are untrusted code and may disclose public or candidate-produced content. Opaque ids are relinkable from public text/order, and this contract neither proves isolated candidate execution, grader correctness, nor task resolution |
| SWE-bench controller-output forgery, run omission, workspace substitution, or process-failure relabeling | The source/sdist-only run ledger replays source/task/preparation/system bindings over the complete physical denominator; requires each caller-provided workspace to match its preparation before launch; writes the exact two-field request; appends request/workspace paths to fixed literal argv; and retains bounded raw stdout/stderr prefixes, process evidence, initial/final workspace summaries, deterministic deltas, and exhaustive dispositions. Captures derive only from replayed complete stdout. Ancestor/link/reparse/hard-link/special-entry checks, non-overlap rules, exclusive artifacts, exact label sets, executable/argv/environment revalidation, self-hash validation, and live replay enforce structural consistency, raw-output-derived captures, retained-summary reconciliation, and denominator accounting | The workspace is not a coordinator-created mount and the literal lifecycle is not a sandbox. A controller can access any host resource allowed to its process and a POSIX child can leave the owned process group; no mount, filesystem, network, user, PID, or image isolation is proved. Workspace traversal and executable launch are pathname-based rather than descriptor-pinned, so concurrent nested-directory or executable substitution remains a host trust boundary. Process exit, trigger, timing, and cleanup fields are unauthenticated coordinator observations and can be relabeled consistently with raw output and a recomputed self-hash. A failed final scan retains no contents for later replay. Executable, argv, environment, expected-system, token, trajectory, raw-output, and self-hash bindings do not authenticate the controller, loaded script/code, model, agent, token counter, trajectory, or producer. Hidden tests, grading, resolution, score, usefulness, and claim readiness remain false; current tests use only synthetic repositories, workspaces, and controllers |
...
| Canonical adapter trust promotion, dependency shadowing, projection loss, or transport drift | Before the adapter initiates package import or exposes a preloaded root, one exact-version distribution, all 35 immutable wheel `RECORD` rows and only the closed installer-generated set, an unset external bytecode-cache prefix, exact built-in module/spec/source-loader state bound to its recorded package, an exact bounded link-free source/resource tree digest and sizes, and absence of unexpected importable entries must agree. One empty-environment no-site batch worker compiles the cache-associated verified source for every record before parsing any cache and, under byte/time/process-tree bounds plus a 256 MiB Windows/non-Darwin or exact 1 TiB Darwin virtual-address-space ceiling, requires complete marshal records, const-stripped format-2 metadata, raw adaptive instruction/cache images, and a bounded tagged constant graph with per-code identity topology; the complete `RECORD`/file/origin gate repeats after import. Exact-wheel PEP 610 metadata and one platform-canonical launcher are mandatory, so a transitive optional-extra install without direct archive binding fails closed. Typed request and NDJSON surfaces then accept actual canonical requests after a wheel-owned handshake, advertise one executed operation, apply identical bounded canonical parsing in-process and over NDJSON, namespace input metadata, require an independent host callback before preserving authority, return every complete source as untrusted evidence, project only exact source quotes with UTF-8 byte offsets, record protected omissions/overflow, and pass the fixed 22-case non-inference gate. Direct `handle` is only the wheel server's negotiated callback seam | The installed `RECORD` and PEP 610 claim do not independently authenticate the wheel archive, and check/import is not atomic against writable site-packages. Prior `.pth`, `sitecustomize`, custom `meta_path`, or preloaded same-origin object effects cannot be undone; a custom finder runs during spec resolution, and the local host/Python process remains trusted. The ten-second limit is the bytecode stage's latest acceptance deadline; mandatory cleanup has separate bounded grace. Cross-process sharing between separate nested code objects is normalized; cacheful validation depends on CPython's private raw adaptive-code image. Worker containment does not provide a filesystem/network sandbox against a native marshal vulnerability. A compromised verifier can authenticate the wrong event. The canonical projection does not prove authenticity, semantic completeness, or final-request accounting and cannot carry the private artifact, archive state, or detector-scoped certificate |
