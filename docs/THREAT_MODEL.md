...
| Secret disclosure | None beyond caller-controlled storage and provider choice | Source quotes, metadata, artifacts, prompts, benchmark JSON, exception diagnostics, opt-in completion events, and model calls can expose secrets; the LM Studio CLI prompt is visible in process arguments on some hosts |
